...
def read_grades_from_summary():
    """
    Read final grades from student_summary.txt generated by Main.py
    ...
    if not os.path.exists(summary_file):
        print(f"[WARN] Warning: {summary_file} not found!")
    ...
    try:
        with open(summary_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                elif line.startswith("GitHub Username:"):
                    username = line.replace("GitHub Username:", "").strip().lstrip('@')
                    if current_repo:
                        grades[current_repo]['github_username'] = username

                # Parse final score
                elif line.startswith("Final Score:"):
                    score_text = line.replace("Final Score:", "").strip()
                    # Extract numeric score (e.g., "88.76" from "88.76/100")
                    match = re.match(r'([\d.]+)/100', score_text)
                    if match and current_repo:
                        grades[current_repo]['final_score'] = float(match.group(1))

                # Parse grade letter
                elif line.startswith("Grade:"):
                    grade = line.replace("Grade:", "").strip()
                    if current_repo:
                        grades[current_repo]['grade'] = grade

        return grades

    except Exception as e:
        print(f"[FAIL] Error reading student summary: {e}")
        return {}
...
def main():
    ...
    grades_data = read_grades_from_summary()
    print(f"[OK] Loaded {len(grades_data)} grade(s) from student_summary.txt")
